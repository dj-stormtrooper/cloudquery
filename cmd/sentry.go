...
func initSentry() {
	...
	dsn := viper.GetString(flags.SentryDSN)
	if viper.GetBool(flags.NoTelemetry) {
		dsn = "" // "To drop all events, set the DSN to the empty string."
	}
	if core.Version == core.DevelopmentVersion && !viper.GetBool(flags.SentryDebug) {
		dsn = "" // Disable Sentry in development mode, unless debug-sentry was enabled
	}
	userId := analytics.GetCookieId()
	if analytics.CQTeamID == userId.String() && !viper.GetBool(flags.SentryDebug) {
		dsn = ""
	}

	if err := sentry.Init(sentry.ClientOptions{
		Debug:     viper.GetBool(flags.SentryDebug),
		Dsn:       dsn,
